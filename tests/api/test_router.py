...
def build_client():
  ...
  app = FastAPI()
  app.include_router(creat_router(chat_service = service))
...
def test_chat_passes_user_and_conversation_to_service():
  ...
  assert service.calls==[("chat","user-a","conversation-1","Hello")]

#验证 API 在面对"无效资源"或"越权访问"时，能否正确地返回统一的 404 状态码
...
def test_update_system_prompt_is_conversation_scoped():
  ...
  response = client.put(
    ...
    json={"user_id":"user-a","system_prompt":"new prompt"}
  )
  assert response.status_code==200
  assert response.json()=={"updated": True}
  assert service.calls[0]==("prompt","user-a","conversation-1","new prompt")
...
def test_blank_question_and_prompt_return_422():
  ...
  response_2 = client.put(
    "/conversations/conversation-1/system-prompt/",
    json={"user_id":"user-a","system_prompt":"    "}
  )
