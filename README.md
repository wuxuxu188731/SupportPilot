...
- Agentic Search 会对每次 embedding 召回且经 SQLite 校验的 chunk 调用百炼 `qwen3-rerank`；传统 RAG Baseline 保持原有排序逻辑，不经过 rerank。
...
重排序默认使用北京业务空间 `ws-tocwkn1wc3xhur1f` 和通用问答检索指令，并仅对 embedding 召回且经 SQLite 校验的 chunk 正文排序。可通过 `KNOWLEDGE_RERANK_INSTRUCT=Retrieve semantically similar text.` 切换为语义相似度策略；密钥始终从 `DASHSCOPE_API_KEY` 读取。
