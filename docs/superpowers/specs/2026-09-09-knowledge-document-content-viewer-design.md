...
非目标（本次不做）：PDF 原文渲染、原文排版还原、原文文件下载、引用在会话历史中的持久化。
...
| 6 | 刷新后引用 | **本次不管**：聊天历史仍不保存 citations（列为后续项） |
| 7 | 原文下载 | 本次**不提供**（列为后续项） |
...
| # | 事项 | 为什么先不做 | 触发条件 / 前置 |
| --- | --- | --- | --- |
| 1 | **引用在会话历史中持久化**（刷新后仍能跳转） | 需要后端在历史接口返回 citations，属于安全历史的接口范围扩张，且要决定历史引用与"当时版本"的关系 | 产品要求"刷新后仍可核对来源"时启动；前置：`ConversationHistoryMessage` 结构扩展 + 前端 stores/chat 改造 |
