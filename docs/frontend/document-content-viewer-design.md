...
> **已由正式设计稿取代：`docs/superpowers/specs/2026-09-09-knowledge-document-content-viewer-design.md`。**
> 本文保留为可行性论证与源码定位附录；评审结论（决策点）已在正式设计稿 §3 定稿，
> 任务拆分与验收标准见其 §5。后续实现请以正式设计稿为准，本文不再更新。
>
...
> 实现结果（2026-09-16）：桌面 **≥1280px** 为对话区右侧内嵌面板；窄屏按上面的
> 「整页路由」落地，并补上了本条提示的代价——跳转时附带
> `from=chat&conversationId=`，详情页正文弹窗底部提供 **「关闭」**（停留详情页）
> 与 **「返回对话」**（回到原会话）。此前窄屏用户点完引用只能退出对话页再重新
> 进入，属于体验缺陷（用户反馈 DEF，已修）。
